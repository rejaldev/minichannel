...
import { productsAPI, transactionsAPI, settingsAPI, cabangAPI, returnsAPI } from '@/lib/api';
...
export default function POSPage() {
  ...
  // Request Product states (WhatsApp)
  const [showRequestProductModal, setShowRequestProductModal] = useState(false);
  const [categories, setCategories] = useState<any[]>([]);
  const [adminWhatsApp, setAdminWhatsApp] = useState('6282112406540');
  ...
  // Auto-focus search & load held transactions
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setHeldTransactions(parsed.map((t: any) => ({ ...t, timestamp: new Date(t.timestamp) })));
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      if (user?.role === 'KASIR') return; // KASIR doesn't need branch selection
      
      try {
        const res = await cabangAPI.getCabangs();
        const activeBranches = res.data.filter((b: any) => b.isActive);
        ...
        if (savedCabangId && activeBranches.some((b: any) => b.id === savedCabangId)) {
          ...
          localStorage.setItem('activeCabangId', activeBranches[0].id);
        }
      } catch (e) {
    ...
    fetchBranches();
  }, [user?.role, user?.cabangId]);

  // Fetch products when cabang changes
  useEffect(() => {
    if (selectedCabangId || user?.role === 'KASIR') {
      fetchProducts();
      fetchSettings();
      fetchCategories();
    }
  }, [search, selectedCabangId]);
  ...
  const fetchProducts = async () => {
    try {
      const res = await productsAPI.getProducts({ search: search || undefined, isActive: true });
      const effectiveCabangId = getEffectiveCabangId();
      ...
      if (effectiveCabangId) {
        const filtered = res.data.filter((product: any) => {
          if (product.productType === 'SINGLE') {
            ...
            return v?.stocks?.some((s: any) => s.cabangId === effectiveCabangId);
          }
          return product.variants?.some((variant: any) => 
            variant.stocks?.some((s: any) => s.cabangId === effectiveCabangId)
          );
        });
  ...
  useProductSocket({
    onProductCreated: useCallback(() => {
      // Refresh products when a new product is created
      fetchProducts();
    }, []),
    onProductUpdated: useCallback((updatedProduct: any) => {
      // Update product in list and cart
      ...
      setCart(prev => prev.map(item => {
        const variant = updatedProduct.variants?.find((v: any) => v.id === item.productVariantId);
        if (variant) {
          const stock = variant.stocks?.find((s: any) => s.cabangId === effectiveCabangId);
          return {
            ...item,
            productName: updatedProduct.name,
            price: stock?.price || item.price,
            availableStock: stock?.quantity || item.availableStock,
          };
        }
        return item;
      }));
    ...
    onProductDeleted: useCallback((productId: string) => {
      ...
      // Note: Cart items from deleted products will show but can't be processed
    }, []),
      ...
      setProducts(prev => prev.map(product => ({
        ...
        variants: product.variants?.map((variant: any) => ({
          ...
          stocks: variant.stocks?.map((stock: any) => 
            stock.cabangId === stockData.cabangId && variant.id === stockData.productVariantId
              ? { ...stock, quantity: stockData.quantity, price: stockData.price ?? stock.price }
              : stock
          )
        }))
      })));
      
      // Update available stock in cart
      setCart(prev => prev.map(item => 
        item.productVariantId === stockData.productVariantId
          ? { ...item, availableStock: stockData.quantity, price: stockData.price ?? item.price }
          : item
      ));
    }, []),
  ...
  const addToCart = (product: any, variant: any) => {
    ...
    const existing = cart.find((item) => item.productVariantId === variant.id);
    if (existing) {
  ...
  const updateQuantity = (variantId: string, newQty: number) => {
    ...
    if (newQty > item.availableStock) { toast.warning('Stok tidak mencukupi!'); return; }
    setCart(cart.map((i) => i.productVariantId === variantId ? { ...i, quantity: newQty } : i));
  };
  ...
  const calculateTotal = () => {
    ...
    return { subtotal, total: subtotal - discountAmount, discountAmount };
  };

  const { subtotal, total, discountAmount } = useMemo(() => calculateTotal(), [cart, discount, discountType]);
  ...
  const handleHoldTransaction = () => {
    ...
    const held: HeldTransaction = {
      ...
      cart: [...cart],
      customerName, customerPhone, paymentMethod, discount, discountType, bankName, referenceNo,
      timestamp: new Date(),
    };
  ...
  const handleRetrieveHeld = (held: HeldTransaction) => {
    const doRetrieve = () => {
      ...
      const updated = heldTransactions.filter(t => t.id !== held.id);
      setHeldTransactions(updated);
      localStorage.setItem('heldTransactions', JSON.stringify(updated));
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setLastCashReceived(paymentMethod === 'CASH' ? cashReceived : 0);
      setShowSuccess(true);
      resetTransaction();
      fetchProducts();
                ...
                cart.map((item) => (
                  <div key={item.productVariantId} className="flex items-center justify-between bg-white dark:bg-gray-700 rounded-xl shadow-sm border border-gray-100 dark:border-gray-600 p-3">
                    ...
                    <div className="flex items-center space-x-1">
                      <button onClick={() => updateQuantity(item.productVariantId, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center bg-gray-100 dark:bg-gray-600 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-500 font-bold text-sm">-</button>
                      <span className="w-8 text-center font-bold text-gray-900 dark:text-white">{item.quantity}</span>
                      <button onClick={() => updateQuantity(item.productVariantId, item.quantity + 1)} className="w-7 h-7 flex items-center justify-center bg-blue-100 dark:bg-blue-600 rounded-lg hover:bg-blue-200 dark:hover:bg-blue-500 font-bold text-sm">+</button>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => setShowSuccess(false)} className="flex-1 py-2.5 bg-slate-600 text-white rounded-lg font-medium hover:bg-slate-700 transition">Transaksi Baru</button>
            </div>
            {!qzConnected && (
